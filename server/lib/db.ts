...
async function connectToDatabase() {  // MongDB 연결 한번만 시도하고, 이미 연결되어 있으면 재사용
    if (cached.conn) {
        return cached.conn;
    }
    ...
    if (!cached.conn) {
        ...
        .then((mongooseInstance) => mongooseInstance.connection)    // Promise<mongoose.Connection>
        .catch((error) => {
